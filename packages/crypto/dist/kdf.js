 ...
 */
export async function deriveKdfKey(pin) {
    ...
    const key = await subtle.importKey('raw', derivedBits, { name: 'AES-GCM' }, false, ['encrypt', 'decrypt']);
    return key;
}
async function deriveArgon2(pin) {
    ...
    const rawAny = await argon2.hash(pin, {
...
async function derivePbkdf2(pin) {
    // Announce fallback — non-secret message required by unit tests.
    console.log('using pbkdf2 fallback');
    const encoder = new TextEncoder();
    const pwKey = await subtle.importKey('raw', encoder.encode(pin), 'PBKDF2', false, ['deriveBits']);
