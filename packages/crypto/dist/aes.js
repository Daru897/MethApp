 ...
 */
export async function encrypt(dataKey, plaintext, associatedData) {
    ...
    const ct = await subtle.encrypt({ name: 'AES-GCM', iv, additionalData: ad }, dataKey, pt);
    return { iv, ct };
}
/**
 ...
 */
export async function decrypt(dataKey, payload, associatedData) {
    ...
    const ad = associatedData && associatedData.length > 0 ? associatedData : undefined;
    const decrypted = await subtle.decrypt({ name: 'AES-GCM', iv: payload.iv, additionalData: ad }, dataKey, payload.ct);
